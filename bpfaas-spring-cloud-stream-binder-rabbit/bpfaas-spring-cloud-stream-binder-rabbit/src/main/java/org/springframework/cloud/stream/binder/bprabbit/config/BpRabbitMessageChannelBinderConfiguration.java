...
import org.springframework.cloud.stream.binder.bprabbit.properties.RabbitBinderConfigurationProperties;
...
public class BpRabbitMessageChannelBinderConfiguration {

	@Autowired
	@Qualifier("bpRabbitConnectionFactory")
	private ConnectionFactory bpRabbitConnectionFactory;

	@Autowired
	private BpRabbitProperties bpRabbitProperties;

	@Autowired
	private RabbitBinderConfigurationProperties rabbitBinderConfigurationProperties;

	@Autowired
	private RabbitExtendedBindingProperties bpRabbitExtendedBindingProperties;

	@Bean
	BpRabbitMessageChannelBinder bpRabbitMessageChannelBinder(
		...
		BpRabbitMessageChannelBinder binder = new BpRabbitMessageChannelBinder(
				this.bpRabbitConnectionFactory, this.bpRabbitProperties,
		...
		binder.setExtendedBindingProperties(this.bpRabbitExtendedBindingProperties);
	...
	RabbitExchangeQueueProvisioner provisioningProvider() {
		return new RabbitExchangeQueueProvisioner(this.bpRabbitConnectionFactory);
	}

	@Bean
	@ConditionalOnMissingBean(ConnectionNameStrategy.class)
	@ConditionalOnProperty("spring.cloud.stream.bprabbit.binder.connection-name-prefix")
