...
package org.springframework.cloud.stream.binder.bprabbit.config;
...
@Import({ BpRabbitMessageChannelBinderConfiguration.class,
		BpRabbitServiceAutoConfiguration.RabbitHealthIndicatorConfiguration.class })
public abstract class BpRabbitServiceAutoConfiguration {

	static void configureCachingConnectionFactory(
			CachingConnectionFactory connectionFactory,
			ConfigurableApplicationContext applicationContext,
			BpRabbitProperties rabbitProperties) throws Exception {
		...
		if (rabbitProperties.getCache().getChannel().getCheckoutTimeout() != null) {
			connectionFactory
					.setChannelCheckoutTimeout(rabbitProperties.getCache().getChannel().getCheckoutTimeout().toMillis());
		}
	...
	@Bean
	BpRabbitProperties bpRabbitProperties() {
		BpRabbitProperties pro = new BpRabbitProperties();
		return pro;
	}

	@Bean("bpRabbitConnectionFactory")
	public CachingConnectionFactory bpRabbitConnectionFactory(ConfigurableApplicationContext applicationContext, BpRabbitProperties rabbitProperties) throws Exception {
		...
		BpRabbitServiceAutoConfiguration.configureCachingConnectionFactory(
							connectionFactory,
	...
	//  * @param rabbitProperties bprabbit properties
	//  * @return the {@link ConnectionFactory} used by the binder.
	//  * @throws Exception if configuration of connection factory fails
	//  */
	// @Bean("bpRabbitConnectionFactory")
	// ConnectionFactory bpRabbitConnectionFactory(Cloud cloud,
	// 											ObjectProvider<RabbitConnectionFactoryConfig> connectorConfigObjectProvider,
	// 											ConfigurableApplicationContext applicationContext, BpRabbitProperties rabbitProperties) throws Exception {
	...
	// 	configureCachingConnectionFactory((CachingConnectionFactory) connectionFactory, applicationContext,
	// 			rabbitProperties);
	...
	protected static class CloudProfile {
		...
		protected static class CloudConnectors {
			...
			@EnableConfigurationProperties(BpRabbitProperties.class)
			protected static class UseCloudConnectors {
				...
				RabbitTemplate rabbitTemplate(@Qualifier("bpRabbitConnectionFactory") ConnectionFactory bpRabbitConnectionFactory) {
					return new RabbitTemplate(bpRabbitConnectionFactory);
				}
